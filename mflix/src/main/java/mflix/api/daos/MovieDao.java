...
public class MovieDao extends AbstractMFlixDao {
	...
	public Document getMovie(String movieId) {
		...
		pipeline.add(match);
		// DONE> Ticket: Get Comments - implement the lookup stage that allows the
		// comments to
	...
	public List<Document> getMoviesByCountry(String... country) {
		...
		// DONE> Ticket: Projection - implement the query and projection required by the
		// unit test
		List<Document> movies = new ArrayList<>();
	...
	public List<Document> getMoviesByGenre(String sortKey, int limit, int skip, String... genres) {
		...
		Bson sort = Sorts.descending(sortKey);
		List<Document> movies = new ArrayList<>();
		// DONE > Ticket: Paging - implement the necessary cursor methods to support
		// simple
